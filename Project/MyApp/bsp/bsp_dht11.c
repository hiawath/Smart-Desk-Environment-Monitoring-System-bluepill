...
static dht11Handle_t *s_active_dht = NULL;

/* ----------------------------------------------------------------
 * 객체 지향 메서드 래퍼 함수들 (dht11_ 네임스페이스)
 * ----------------------------------------------------------------*/

static bool dht11_read(dht11Data_t *data)
{
  return dht11Read(s_active_dht, data);
}

static float dht11_getTemperature(void)
{
  return dht11GetTemperature(s_active_dht);
}

static float dht11_getHumidity(void)
{
  return dht11GetHumidity(s_active_dht);
}
...
void dht11Init(dht11Handle_t *hdht, const dht11Pin_t *pins)
{
  ...
  /* 메서드 함수 포인터 바인딩 */
  hdht->read           = dht11_read;
  hdht->getTemperature= dht11_getTemperature;
  hdht->getHumidity   = dht11_getHumidity;
