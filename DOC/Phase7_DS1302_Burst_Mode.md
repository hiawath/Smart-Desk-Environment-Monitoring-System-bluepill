...
## 4. 추가 분석: 버스트 내부 바이트 간 핀 충돌(Bus Contention) 해결

### 4.1 원인: 바이트 단위 DAT 모드 전환 버그
- 1차 버스트 모드 적용 후에도 초가 엇갈렸던 근본 원인은 `ds1302ReadByte()` 내부에서 매 바이트 수신 후 `ds1302SetDatOutput()`을 호출했던 데 있었음.
- 버스트 모드에서는 8바이트가 끊김 없이 연속으로 클럭킹되어야 하는데, 1바이트(Seconds)가 끝날 때마다 MCU가 DAT 라인을 `OUTPUT_PP`로 변경하여 DS1302가 다음 바이트(Minutes)를 출력하려는 라인과 정면 충돌(Bus Contention)이 발생함.
- 그 결과 초(Seconds: 23, 24, 25)와 분(Minutes: 51, 52, 53)의 비트가 경계에서 섞여 엇갈려 나타남.

### 4.2 최종 조치
1. `ds1302ReadByte()`는 순수하게 클럭 쉬프트만 수행하도록 변경(핀 모드 제어 코드 완전 제거).
2. `ds1302ReadBurstClock()`에서 8바이트 전체를 수신하는 동안 `ds1302SetDatInput()` 상태를 단 1회 설정하여 8바이트 전체 수신 완료 시까지 안정적으로 유지하도록 수정.
